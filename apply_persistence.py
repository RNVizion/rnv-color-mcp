#!/bin/bash
# apply_persistence.sh  -  durable palettes via HF Dataset write-through (+ fixes /data path)
# Run from the repo root in your Codespace:
#     bash apply_persistence.sh <HF_WRITE_TOKEN>
# or set HF_TOKEN as a Codespace secret and run:  bash apply_persistence.sh
set -e

TOKEN="${HF_TOKEN:-$1}"
if [ -z "$TOKEN" ]; then
  echo "Need a Hugging Face WRITE token: bash apply_persistence.sh <token>"
  exit 1
fi

echo "1/4  Writing engine/palette_store.py ..."
cat > engine/palette_store.py << 'PYEOF'
"""
...
echo "3/4  Pointing the local store off the read-only /data path ..."
sed -i 's|ENV RNV_PALETTE_STORE=/data/palettes.json|ENV RNV_PALETTE_STORE=/home/user/app/data/palettes.json|' Dockerfile

echo "4/4  Commit + push to the Space ..."
BASE=$(git remote get-url space | sed 's|https://[^@]*@|https://|')
USER=$(echo "$BASE" | cut -d/ -f5)
git add -A
git commit -m "Durable palettes via HF Dataset write-through; fix /data path" || true
git push "$(echo "$BASE" | sed "s|https://|https://${USER}:${TOKEN}@|")" HEAD:main --force

echo ""
echo "Done. Watch the Space rebuild to Running, then re-run the save test."
echo "First save should return durable: true, and a private dataset <user>/rnv-color-palettes appears."
